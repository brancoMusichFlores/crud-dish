...
def handler(event, context):
    ...
    if 'Item' in existing_subscriber:
        return {
            "statusCode": 400,
            "body": json.dumps({"msg":f"Ya existe un suscriptor con el numero de telefono {cellphone_number}."})
        }
    if 'apellido_paterno' not in subscriber:
        subscriber['apellido_paterno'] = ""       
    # add subscriber to dynamo table
    table.put_item(Item=subscriber)
    response = {
        "statusCode": 200, 
        "body": json.dumps({"msg":"Suscriptor creado exitosamente!"})
    }
